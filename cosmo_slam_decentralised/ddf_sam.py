...
class DDFSAMBackend:
    ...
    def ingest_factors(self, factors: Iterable[BetweenFactorPose3 | PriorFactorPose3]) -> None:
        ...
        # Track inter-robot factors we've already accounted for so KPI/bandwidth
        # metrics match a single transmission rather than per-robot duplication.
        _seen_inter_ids = set()
        ...
        if self.kpi or self.bandwidth or self.latency or self._ingest_pacer.enabled:
            for rid, bundle in self._bundles.items():
                ...
                for f in bundle.inter_between:
                    ...
                    rid1 = self.robot_map.get(str(f.key1), rid)
                    rid2 = self.robot_map.get(str(f.key2), rid)
                    inter_id = (tuple(sorted((str(f.key1), str(f.key2)))), float(stamp))
                    first_time = inter_id not in _seen_inter_ids
                    if first_time:
                        _seen_inter_ids.add(inter_id)
                    topic = f"between/{rid1}-{rid2}"
                    size = factor_bytes(f)
                    if self.bandwidth and first_time:
                        self.bandwidth.add_uplink(topic, size)
                    if self.latency and first_time:
                        try:
                            ev_id = self.latency.record_ingest(
                                "BetweenFactorPose3",
                                stamp,
