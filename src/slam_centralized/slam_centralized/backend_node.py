"""Centralized SLAM back‑end using GTSAM.

This node subscribes to IMU and odometry (ICP) topics, constructs a
factor graph with IMU pre‑integration and pose constraints, performs
incremental smoothing via ISAM2, detects simple loop closures based on
translation distance and publishes the resulting optimized path as a
``nav_msgs/Path``.
"""
...
def pose_to_gtsam(msg: PoseStamped) -> gtsam.Pose3:
    """Convert a ROS ``PoseStamped`` into a GTSAM ``Pose3``."""
    t = msg.pose.position
...
class CentralBackend(Node):
    """Fuse IMU and ICP measurements into an optimized trajectory using GTSAM."""

    def __init__(self) -> None:
        super().__init__('central_node')

        # Parameters for agent name and odometry topic
        ...
        self.saved_poses: OrderedDict[int, gtsam.Pose3] = OrderedDict()
        self.looped_pairs: set[tuple[int, int]] = set()
        ...
        self.imu_bias = imuBias.ConstantBias()
        self.preintegrator = gtsam.PreintegratedImuMeasurements(imu_params, self.imu_bias)
        self.prev_imu_time: float | None = None

        # Subscribers and synchronizer
        imu_sub = Subscriber(self, Imu, 'imu/data')
        icp_sub = Subscriber(self, PoseStamped, self.odom_topic)
        ats = ApproximateTimeSynchronizer([imu_sub, icp_sub], queue_size=10, slop=0.1)
        ats.registerCallback(self.sync_callback)

        # Publisher for the optimized path
        self.path_pub = self.create_publisher(Path, 'path', 10)

        self.get_logger().info(
            f"[{self.agent}] CentralBackend ready: listening on 'imu/data' & '{self.odom_topic}', publishing on 'path'"
        )

    def sync_callback(self, imu: Imu, icp: PoseStamped) -> None:
        """Handle synchronized IMU and ICP messages."""
        # 1) Logging
        self.get_logger().info(
        ...
        if self.pose_idx == 0:
            self.graph.add(
                gtsam.PriorFactorPose3(gtsam.symbol('x', 0), gtsam.Pose3(), self.prior_noise)
            )
            self.initial.insert(gtsam.symbol('x', 0), gtsam.Pose3())

        # 3) IMU pre‑integration and priors
        ...
        if self.pose_idx == 0:
            ...
            self.graph.add(
                gtsam.PriorFactorVector(gtsam.symbol('v', 0), self.current_vel, self.vel_prior_noise)
            )
            self.graph.add(
                gtsam.PriorFactorConstantBias(gtsam.symbol('b', 0), self.current_bias, self.bias_prior_noise)
            )
            ...
            if dt_sum > 0.0:
                ...
                # Fallback priors when no IMU integration happened
                self.get_logger().warn(
                    f"[{self.agent}] No IMU integration (Δt=0) → adding weak priors on v{self.pose_idx} & b{self.pose_idx}"
                )
                self.graph.add(
                    gtsam.PriorFactorVector(
                        gtsam.symbol('v', self.pose_idx), self.current_vel, self.vel_prior_noise
                    )
                )
                self.graph.add(
                    gtsam.PriorFactorConstantBias(
                        gtsam.symbol('b', self.pose_idx), self.current_bias, self.bias_prior_noise
                    )
                )

        # 4) Add between factor from ICP
        ...
        # 5) Loop‑closure detection based on translation distance
        for past_idx, past_pose in list(self.saved_poses.items()):
            # Skip very recent poses to avoid spurious closures
            if self.pose_idx + 1 - past_idx < 20:
                continue
            # Compute Euclidean distance between past pose and current guess
            dist = np.linalg.norm(
                np.array(past_pose.translation()) - np.array(guess.translation())
            )
        ...
        # 6) ISAM2 update and clear temporary factors/values
        self.isam.update(self.graph, self.initial)
        self.graph.resize(0)
        self.initial.clear()

        # 7) Extract optimized state
        ...
        # Reset preintegrator for next segment
        self.preintegrator.resetIntegrationAndSetBias(self.current_bias)
        self.prev_imu_time = None

        # 8) Append the optimized pose to the Path and publish
        stamped = PoseStamped()
        ...
        self.get_logger().info(
            f"[{self.agent}] Published optimized path length: {len(self.path_msg.poses)}"
        )

        # 9) Store pose for future loop closures
        self.saved_poses[self.pose_idx + 1] = self.current_pose
        if len(self.saved_poses) > 100:
            # Remove the oldest pose to keep memory bounded
            self.saved_poses.popitem(last=False)

        # 10) Increment pose index
        self.pose_idx += 1


def main(args=None) -> None:
    """Entry point for the ``backend_node`` executable."""
