...
def _load_config_file(path: Path) -> Dict[str, Any]:
    ...
    if isinstance(raw.get("decentralised"), dict):
        put(raw["decentralised"], {
            ...
            "ddf_convergence": "ddf_convergence",
            "ddf_rot_convergence": "ddf_rot_convergence",
            "ddf_local_iters": "ddf_local_iters",
            "ddf_relaxation": "ddf_relaxation",
            "skip": "skip_decentralised",
            # Per-agent factor streaming (optional)
            "agent_factor_ros2": "ddf_agent_factor_ros2",
            "agent_solve_every_n": "ddf_agent_solve_every_n",
            "agent_solve_period_s": "ddf_agent_solve_period_s",
            "agent_factor_prefix": "ddf_agent_factor_prefix",
            "agent_factor_idle_timeout": "ddf_agent_factor_idle_timeout",
            "agent_factor_reliability": "ddf_agent_factor_reliability",
            "agent_factor_durability": "ddf_agent_factor_durability",
            "agent_factor_depth": "ddf_agent_factor_depth",
        })
...
def parse_args() -> argparse.Namespace:
    ...
    # Optional per-agent factor streaming for decentralised runs
    ap.add_argument(
        "--ddf-agent-factor-ros2",
        action="store_true",
        help="Enable per-agent factor ingest over ROS 2 (agents subscribe to <prefix>/<rid>)",
    )
    ap.add_argument(
        "--ddf-agent-solve-every-n",
        type=int,
        default=0,
        help="During streaming, trigger a local solve after every N factors (0 disables)",
    )
    ap.add_argument(
        "--ddf-agent-solve-period-s",
        type=float,
        default=0.0,
        help="During streaming, trigger a local solve every T seconds (0 disables)",
    )
...
def main() -> int:
    ...
    for rep_idx in range(replicas):
        ...
        ddf_cmd = [
            ...
            "--ddf-convergence", str(float(args.ddf_convergence)),
            "--ddf-rot-convergence", str(float(args.ddf_rot_convergence)),
            "--ddf-local-iters", str(int(args.ddf_local_iters)),
            "--ddf-relaxation", str(float(args.ddf_relaxation)),
        ] + common_solver + ddf_qos_opts

        # If per-agent factor streaming is requested, wire through matching flags.
        if getattr(args, "ddf_agent_factor_ros2", False):
            ddf_cmd += [
                "--ddf-agent-factor-ros2",
                "--ddf-agent-factor-prefix", args.factor_topic,
                "--ddf-agent-factor-reliability", args.qos_reliability,
                "--ddf-agent-factor-durability", args.qos_durability,
                "--ddf-agent-factor-depth", str(int(args.qos_depth)),
                "--ddf-agent-factor-idle-timeout", str(float(args.publisher_wait_timeout)),
            ]
            if int(getattr(args, "ddf_agent_solve_every_n", 0) or 0) > 0:
                ddf_cmd += [
                    "--ddf-agent-solve-every-n",
                    str(int(args.ddf_agent_solve_every_n)),
                ]
            if float(getattr(args, "ddf_agent_solve_period_s", 0.0) or 0.0) > 0.0:
                ddf_cmd += [
                    "--ddf-agent-solve-period-s",
                    str(float(args.ddf_agent_solve_period_s)),
                ]
        ...
        if args.dry_run:
            ...
            if not args.skip_decentralised:
                ...
                if getattr(args, "ddf_agent_factor_ros2", False):
                    print(f"[orchestrate]{rep_label}DDF FACTOR PUBLISHER:")
                    print("  ", _cmd_str(publisher_cmd))
        ...
        if not args.skip_decentralised:
            ...
            # If agents expect factor streams, start the publisher too
            pub_ddf = None
            if getattr(args, "ddf_agent_factor_ros2", False):
                time.sleep(2.0)  # allow subscribers to come up
                env_pub_ddf = _env_for(out_ddf)
                pub_ddf = _spawn(publisher_cmd, cwd=root, env=env_pub_ddf)
                children.append(("factor_publisher_ddf", pub_ddf))

            # Wait for publisher (if any) and the DDF run
            if pub_ddf is not None:
                rc_pub_ddf = _wait(pub_ddf, "factor_publisher_ddf")
            else:
                rc_pub_ddf = 0
            rc_ddf = _wait(ddf, "ddf_mp")
            children.clear()
            overall_rc |= (rc_ddf != 0) or (rc_pub_ddf != 0)
            if rc_ddf == 0 and rc_pub_ddf == 0:
                ...
                print(f"[orchestrate]{rep_label}Decentralised run failed (ddf={rc_ddf}, publisher={rc_pub_ddf})")
